...
class CepTest < Minitest::Test
  ...
  ceps.each do |cep, expected_code|
    assert_equal expected_code, Cep.new(cep).state_code
  end
end

  def test_rio_pardo
    rio_pardo = Cep.new('13.720-000')
  ...
  def test_ibirapuera
    ...
    reasonable_formats.each do |format|
      assert_equal ibirapuera, Cep.new(format)
    end

    unreasonable_formats.each do |format|
      refute_equal ibirapuera, Cep.new(format)
    end
